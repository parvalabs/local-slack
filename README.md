...
- **Unread indicators** — channels with unseen messages go bold, threads with unseen replies get a dot on their reply summary (opening a channel doesn't clear its threads, matching Slack), an unread count shows in the browser tab title, and a mutable chime plays on incoming messages
